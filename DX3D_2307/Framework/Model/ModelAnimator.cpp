...
void ModelAnimator::GUIRender()
{
    ...
    //ImGui::SliderInt("Clip", &frameBuffer->GetData()->clip, 0, clips.size() - 1);
    //ImGui::SliderInt("Frame", (int*)&frameBuffer->GetData()->curFrame,
...
Matrix ModelAnimator::GetTransformByNode(int nodeIndex)
{
    ...
    {
        Frame& curFrame = frameBuffer->GetData()->cur;
    ...
    {
        Frame& nextFrame = frameBuffer->GetData()->next;
        ...
        return Lerp(curAnim, nextAnim, frameBuffer->GetData()->tweenTime);
    }
}
...
void ModelAnimator::UpdateFrame()
{
    ...
    Motion* frameData = frameBuffer->GetData();
