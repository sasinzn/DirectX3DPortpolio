...
void ModelAnimatorInstancing::PlayClip(UINT instanceIndex, int clip, float scale, float takeTime)
{
    ...
    frameInstancingBuffer->GetData()->motions[instanceIndex].takeTime = takeTime;    
...
Matrix ModelAnimatorInstancing::GetTransformByNode(UINT instanceIndex, int nodeIndex)
{
    ...
    {
        Frame& curFrame = frameInstancingBuffer->GetData()->motions[instanceIndex].cur;
    ...
    {
        Frame& nextFrame = frameInstancingBuffer->GetData()->motions[instanceIndex].next;
        ...
        return Lerp(curAnim, nextAnim, frameInstancingBuffer->GetData()->motions[instanceIndex].tweenTime);
    }
}

void ModelAnimatorInstancing::UpdateFrame(UINT instanceIndex, Motion* motion)
{
    {
...
void ModelAnimatorInstancing::UpdateTransforms()
{
    ...
    FOR(transforms.size())
    {
        ...
        {
            UpdateFrame(i, &frameInstancingBuffer->GetData()->motions[i]);
