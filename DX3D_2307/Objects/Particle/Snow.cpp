...
Snow::Snow()
{
    ...
    material->SetDiffuseMap(L"Textures/FX/Snow.png");
...
void Snow::Create()
{
    ...
    FOR(particleCount)
    {
        ...
        Float3 pos;
        pos.x = Random(-buffer->GetData()->size.x, buffer->GetData()->size.x);
        pos.y = Random(-buffer->GetData()->size.y, buffer->GetData()->size.y);
        pos.z = Random(-buffer->GetData()->size.z, buffer->GetData()->size.z);
